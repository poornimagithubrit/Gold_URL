...
app = FastAPI(
    title="Gold Data API",
    description="Public JSON API for gold sales",
    version="1.0"
)
...
@app.get("/data")
def get_data():
    """Return the full CSV content as JSON."""
    return JSONResponse(content=df.to_dict(orient="records"))

@app.get("/filter")
def filter_data(city: str = None, category: str = None):
    """Filter by city and/or master category."""
    filtered_df = df.copy()
    if city:
...
@app.get("/summary")
def summary():
    """Return total sales and average cost price."""
